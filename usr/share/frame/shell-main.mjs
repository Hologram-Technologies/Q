      ...
      import HoloCtx from "/_shared/holo-ctx.mjs";   // the ONE context-menu kernel (HOLO-CTX-ONE): native mirror + paint-before-action, shared by every surface
      ...
      defineBlock("holo-window", {
        ...
        style: `:host{display:block;filter:drop-shadow(0 14px 34px #000b);will-change:transform;contain:layout}
          ...
          /* FRAMED holospace (app) windows auto-hide their titlebar too — the app-name + window controls
             fade in only on hover, so a framed app reads as pure content at rest (the frame + border stay).
             The nav overlays the top edge; the body reclaims the full height. pointer-events:none while
             hidden so the app's own top strip stays clickable. Frameless/nested/max keep their own rules. */
          :host([data-kind="app"]:not([frameless]):not([nested])) .nav{position:absolute;top:0;left:0;right:0;z-index:8;opacity:0;pointer-events:none;transition:opacity .14s ease;background:#161b22e6;backdrop-filter:blur(6px);border-bottom:0}
          :host([data-kind="app"]:not([frameless]):not([nested]):hover) .nav{opacity:1;pointer-events:auto}
      ...
      // A1 (holo-wallet-resident): the wallet stops being a window. A signed-in operator's shell answers
      // the ONE signing seam residently — vault opens LAZILY on first credential use (display-split), every
      // value act still one payload-bound biometric. Guests attach nothing (fail-closed). Fail-soft + lazy.
      const bootResidentWallet = async () => {
        try {
          const op = HoloSession.signedInOperator && HoloSession.signedInOperator();
          if (!op || !op.kappa || !op.cred) return;
          const { startResidentWallet } = await import("/_shared/holo-wallet-resident-host.mjs");
          startResidentWallet({ operator: op });
        } catch (e) { console.warn("holo-wallet-resident: not attached —", e && e.message); }
      };
      try { bootResidentWallet(); } catch (e) {}                      // warm shell / roam: operator already signed in
      ...
      async function onSignIn(operator, secret) {
        try {
          ...
          try { bootResidentWallet(); } catch (e) {}   // A1: operator just signed in → the wallet seam goes resident
      ...
      (function wireWallet() {
        ...
        function ensure() {
          ...
          aside = createAside({ id: "wallet", title: "Wallet", onClose: () => {
            ...
            setTimeout(() => { if (aside && !aside.isOpen()) aside.unmount(); }, 360);   // release the stream → wallet relocked
          }});
        ...
        async function mount() {
          ...
          // holospace (default-deny preserved, Law-aligned security). aside.mount (the ONE stream slot)
          // applies sandbox/allow BEFORE src, so the gate holds from the first byte; the fresh #hash each
          // open forces a reload → locked → biometric every open.
          ...
          frame = aside.mount("/wallet.html?panel=1#" + Date.now(), { title: "Holo Wallet",
            ...(sandbox ? { sandbox } : {}),
            allow: (allow ? allow + "; " : "") + "publickey-credentials-get *; publickey-credentials-create *; clipboard-write" });
          return true;
        }
      ...
      // ── Holo Money → the NEOBANK surface, same right carriage as the Wallet. Wallet = the vault
      //    (keys, biometric gate); Money = the bank UX over it (balance · cards · pay · stats). It is
      //    an apps-catalog holospace (/apps/holo-money), sandboxed + capability-gated like any app;
      //    it talks to the wallet only through the BroadcastChannel gate (default-deny preserved).
      //    One responsive column: this carriage on desktop IS the phone app on mobile. ──
      (function wireMoney() {
        const idBtn = $("#identity-btn"); if (!idBtn || !idBtn.parentElement) return;
        // chrome trigger — a calm card glyph LEFT of the identity stamp, same icon-button grammar
        const b = document.createElement("button");
        b.id = "money-btn"; b.type = "button"; b.title = "Holo Money"; b.setAttribute("aria-label", "Holo Money");
        b.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="6" width="18" height="13" rx="2.5"/><path d="M3 10.5h18"/><path d="M7 15h4"/></svg>';
        const st = document.createElement("style");
        st.textContent = "#money-btn{flex:0 0 auto;width:var(--icon-tap);height:var(--icon-tap);margin-left:var(--sp-2);align-self:center;display:grid;place-items:center;border:0;border-radius:9px;background:transparent;color:var(--holo-ink-dim);cursor:pointer;transition:background .12s ease,color .12s ease,transform .12s ease}#money-btn:hover{background:color-mix(in srgb,var(--holo-ink) 10%,transparent);color:var(--holo-ink)}#money-btn:active{transform:scale(.92)}#money-btn.on{color:var(--holo-ink);background:color-mix(in srgb,var(--holo-ink) 12%,transparent)}#money-btn svg{width:var(--icon-md);height:var(--icon-md)}";
        document.head.appendChild(st);
        idBtn.parentElement.insertBefore(b, idBtn);
        let aside = null, frame = null, busy = false;
        function ensure() {
          if (aside) return aside;
          aside = createAside({ id: "money", title: "Money", onClose: () => {
            b.classList.remove("on"); b.setAttribute("aria-expanded", "false");
            setTimeout(() => { if (aside && !aside.isOpen()) aside.unmount(); }, 360);   // release the stream → nothing lingers
          }});
          return aside;
        }
        async function mount() {
          const def = { name: "Holo Money", capabilities: { storage: ["org.hologram.HoloMoney"] } };
          const { sandbox, allow } = capabilitiesToSandbox(await gateCaps(def));   // declared ∩ agreed
          frame = aside.mount("/apps/holo-money/index.html?panel=1#" + Date.now(), { title: "Holo Money",   // fresh mount, live reads only — the wallet still gates every send
            ...(sandbox ? { sandbox } : {}),
            allow: (allow ? allow + "; " : "") + "publickey-credentials-get *; clipboard-write" });
          return true;
        }
        async function toggle(next) {
          ensure();
          ...
          if (next) {
            if (busy || aside.isOpen()) return; busy = true;
            const ok = await mount(); busy = false; if (!ok) return;
            aside.open(); b.classList.add("on"); b.setAttribute("aria-expanded", "true");
          } else {
            aside.close();
          }
        }
        b.onclick = () => toggle();
        window.HoloMoney = { toggle: () => toggle(), open: () => toggle(true), close: () => toggle(false), isOpen: () => !!(aside && aside.isOpen()) };
      })();
      ...
      (function wireIdentityBridge() {
        addEventListener("message", (e) => {
          ...
          if (a === "open-wallet") { try { window.HoloWallet && window.HoloWallet.open(); } catch (_) {} }   // Money (vault)
          else if (a === "open-money") { try { window.HoloMoney && window.HoloMoney.open(); } catch (_) {} }   // Holo Money (neobank carriage)
          else if (a === "close-money") { try { window.HoloMoney && window.HoloMoney.close(); } catch (_) {} }
      ...
      // The engine that used to live here (renderMenu/openKid/showCtx + marks + instant scheduling)
      // is now the OS-wide kernel — holo-ctx.mjs (HOLO-CTX-ONE S0). This shell only declares menus
      // as item DATA (deskCtxItems · window · folder · picker items); the kernel renders them
      // natively (mac ✓ text-only vs win/linux ● + icons) and enforces paint-before-action (L1).
      const showCtx = (x, y, items) => HoloCtx.open(x, y, items);
      const hideCtx = () => HoloCtx.close();
      ...
      // ── PLAYGROUND (⌘/Ctrl-E) — every element becomes a tappable κ-object; edits mint new κ and the
      //    prism refracts the shell onto YOUR version (HOLO-PLAYGROUND Y2). Toggle on/off; Esc exits. ──
      addEventListener("keydown", async (e) => { if (!(e.metaKey || e.ctrlKey) || e.shiftKey || e.altKey) return; if ((e.key || "").toLowerCase() !== "e") return; const t = e.target; if (/^(INPUT|TEXTAREA)$/.test(t.tagName) || t.isContentEditable) return; e.preventDefault();
        try { const { togglePlayground } = await import("/_shared/holo-playground.mjs"); await togglePlayground(); } catch (x) { console.warn("playground:", x && x.message); } });
      ...
      // Which desktop the user is on + native shortcut rendering — the KERNEL's one detector (L2).
      const hostOS = () => HoloCtx.os();
      const fmtKey = (sc, os) => HoloCtx.key(sc, os);
      ...
      function deskCtxItems() {
        const os = hostOS(), mac = os === "mac";                  // icons are dropped on macOS by the kernel (holo-ctx.mjs L2), so pass them freely
      ...
      // menu dismissal (click-out · Escape · scroll · resize) is owned by the kernel (holo-ctx.mjs)
      ...
      // CASA — bucket the LIVE catalog into meaningful groups (People · Money · Machines · Web · Stuff),
      // first-match wins; anything unmatched falls to "More". Purely a presentation of the same catalog the
      // rest of the shell opens — no second registry. Empty groups are dropped so the room stays calm.
      function casaGroups(cat) {
        const G = [
        ...
        const out = G.map(([l]) => [l, []]); const more = [];
        for (const a of (cat || [])) {
          const s = ((a.name || "") + " " + (a.id || "") + " " + (a.kw || "")).toLowerCase();
          const idx = G.findIndex(([, re]) => re.test(s));
          if (idx < 0) more.push(a); else out[idx][1].push(a);
        }
        const res = out.filter(([, arr]) => arr.length).map(([l, arr]) => [l, arr.slice(0, 10)]);
        if (more.length) res.push(["More", more.slice(0, 10)]);
        return res;
      }
      function renderDiscover() {
        ...
        // CASA (additive): the promise at the very top + your world grouped below — same catalog, calmly framed.
        try {
          const disc = host.querySelector(".disc");
          if (disc) {
            const n = catalog.length;
            const pulse = document.createElement("div");
            pulse.style.cssText = "display:flex;align-items:center;gap:.6rem;flex-wrap:wrap;margin:0 0 20px;padding:10px 14px;border:1px solid var(--holo-border,#2a2a2e);border-radius:12px;background:color-mix(in srgb,var(--accent,#5b8cff) 6%,transparent);font-size:14px;color:var(--holo-ink-dim,#9aa0a6)";
            pulse.innerHTML = '<span style="display:inline-flex;align-items:center;gap:.4rem;font-weight:600;color:var(--holo-ink,#e6e6e6)"><span style="width:8px;height:8px;border-radius:50%;background:#22d3ee;box-shadow:0 0 8px #22d3ee"></span><span class="casa-word">Everything’s yours</span></span>' + (n ? '<span>' + n + ' apps · yours, on this machine</span>' : '') + '<span style="margin-left:auto;display:inline-flex;align-items:center;gap:.35rem">✓ untouched</span>';
            disc.insertBefore(pulse, disc.firstChild);
            // health upgrade — NEVER faked: the word only changes if a real source (HoloSysHealth / Q) answers.
            (async () => { try { const src = window.HoloSysHealth || window.Q; const fn = src && (src.summary || src.health); if (!fn) return; const r = await fn.call(src); if (!r) return; const ok = r.ok === true || r.healthy === true || /\b(ok|good|healthy|green)\b/i.test(String(r.status || r.state || r.level || "")); const w = pulse.querySelector(".casa-word"); if (w) w.textContent = ok === false ? "Needs a look" : "All good"; } catch (e) {} })();
            const groups = casaGroups(catalog);
            if (groups.length) {
              const wrap = document.createElement("div"); wrap.style.cssText = "margin-top:26px";
              wrap.innerHTML = groups.map(([label, apps]) =>
                '<div style="margin-bottom:20px"><div style="font-size:13px;font-weight:700;color:var(--holo-ink-dim,#9aa0a6);margin:0 0 10px">' + esc(label) + '</div>' +
                '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px">' +
                apps.map((a) => '<button class="casa-tile" data-app-id="' + esc(a.id) + '" style="display:flex;align-items:center;gap:.6rem;padding:10px 12px;border:1px solid var(--holo-border,#2a2a2e);border-radius:12px;background:var(--holo-surface-2,#1a1a1c);color:var(--holo-ink,#e6e6e6);cursor:pointer;text-align:left;font:inherit">' +
                  '<span style="width:30px;height:30px;flex:none;display:grid;place-items:center">' + appIconHTML(a.id, a.did) + '</span>' +
                  '<span style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:600;font-size:14px">' + esc(a.name || a.id) + '</span>' +
                  '</button>').join("") +
                '</div></div>'
              ).join("");
              disc.appendChild(wrap);
              [...wrap.querySelectorAll(".casa-tile")].forEach((el) => el.onclick = () => { const a = catalog.find((x) => x.id === el.dataset.appId); if (a) launch(a); });
            }
          }
        } catch (e) {}
      }
      ...
      function csEnsureStyle() {
        ...
        const s = document.createElement("style"); s.id = "cs-style"; s.textContent = `
          ...
          /* THE STANDARD SURFACE (holo-aside.mjs) — the flat shell frame the left nav rail wears: no blur,
             no acrylic, no cast shadow; one 11% hairline on the leading edge; ONE 14px type size. */
          #create-studio{position:fixed;top:0;right:0;bottom:0;left:auto;width:var(--ha-gw);z-index:60;
            display:flex;flex-direction:column;background:var(--holo-frame-bg,#141417);color:var(--holo-ink,#e7e7ea);
            border-left:0;
            font:500 var(--ha-fs,14px)/1.5 var(--win-font,ui-sans-serif,system-ui);transform:translateX(26px);
        ...
        async function ensureCodegen(onProgress) {
          ...
          try {
            ...
            if (!window.__holoCoreBrains) {                                // once per shell: bind + background-load the core brains (lazy → no boot cost)
              ...
              // THINK (9B deep-reasoning): mounted only on capable hardware (holo-q-think-tier === "full"), and the
              // brain runs on its OWN resident WebGPU engine (createHoloModelBrain routes qwen35 → holo-qwen35-gpu-real).
              // BOUND but not warmed at boot (shell-perf law); a "Think" mode / model-steer calls .think.kick() on demand.
              const makeThink = async () => { const b = await import("/_shared/voice/holo-voice-holo-brain.mjs"); return (b.createHoloModelBrain || b.default)({ model: "qwen3.5-9b-thinking" }); };
              let canThink = false; try { const tt = await import("/_shared/q/holo-q-think-tier.mjs"); canThink = tt.thinkingTier(await tt.probeThinkingProfile()).tier === "full"; } catch (e) { canThink = false; }
              window.__holoCoreBrains = cb.mountCoreBrains(mux, { makeText: makeText, makeCode: makeCode, makeThink: makeThink, hasGPU: hasGPU, canThink: canThink, onProgress: onProgress });
            }
      ...
      function playInjectStyle() {
        ...
        // THE STANDARD SURFACE — Play inherits the shared holo-aside frame (flat #141417, 11% hairline,
        // ONE 14px type size); the selected app's hue lives only in the CONTENT (the cover card), never
        // on the carriage chrome, so every right frame reads as the same object.
        st.textContent = "#play-studio .pl-rail{--phi:1.618;--s1:8px;--s2:13px;--s3:21px;--s4:34px;position:relative;display:flex;flex-direction:column;gap:var(--s2);height:100%;min-height:0;padding:var(--s3);color:var(--holo-ink,#e7e7ea);font:500 var(--ha-fs,14px)/1.5 var(--win-font,ui-sans-serif,system-ui);overflow:hidden}"
          + "#play-studio .pl-head{flex:0 0 auto;display:flex;align-items:center;justify-content:space-between}"
          + "#play-studio .pl-title{font-size:var(--ha-fs,14px);font-weight:600;letter-spacing:-.006em}"
          + "#play-studio .pl-collapse{width:34px;height:34px;border-radius:10px;border:0;background:transparent;color:var(--holo-ink-dim,#9a9aa2);cursor:pointer;font-size:19px}"
          + "#play-studio .pl-collapse:hover{background:color-mix(in srgb,var(--holo-ink,#e7e7ea) 12%,transparent);color:var(--holo-ink,#e7e7ea)}"
          // search + categories in the shared control grammar — flat surface tiles, hairline edges
          + "#play-studio .pl-srow{flex:0 0 auto;display:flex;gap:var(--s1);align-items:center}"
          + "#play-studio #pl-search{flex:1;min-width:0;padding:10px 16px;font:inherit;color:inherit;background:rgba(255,255,255,.06);border:1px solid var(--holo-border,rgba(255,255,255,.11));border-radius:999px;transition:border-color .2s,box-shadow .2s}"
          + "#play-studio #pl-search::placeholder{color:var(--holo-ink-dim,#9a9ab0)}"
          + "#play-studio #pl-search:focus{outline:none;border-color:var(--accent,#5b8cff)}"
          + "#play-studio .pl-mic{flex:0 0 auto;width:38px;height:38px;border-radius:50%;border:1px solid var(--holo-border,rgba(255,255,255,.11));background:rgba(255,255,255,.06);color:inherit;cursor:pointer;font-size:var(--ha-fs,14px)}"
          ...
          + "#play-studio .pl-chip{flex:0 0 auto;padding:8px 14px;font-size:var(--ha-fs,14px);font-weight:600;border-radius:999px;border:1px solid var(--holo-border,rgba(255,255,255,.11));background:rgba(255,255,255,.06);color:var(--holo-ink-dim,#cdcdd8);cursor:pointer;white-space:nowrap;transition:.18s}"
          + "#play-studio .pl-chip:hover{color:var(--holo-ink,#fff)}"
          + "#play-studio .pl-chip.on{color:#fff;background:var(--accent,#5b8cff);border-color:var(--accent,#5b8cff)}"
          + "#play-studio .pl-card{flex:1 1 auto;min-height:0;display:flex;flex-direction:column;gap:var(--s2);overflow:hidden}"
          + "#play-studio .pl-empty{margin:auto;text-align:center;color:var(--holo-ink-dim,#85858f);font-size:var(--ha-fs,14px);display:flex;flex-direction:column;gap:var(--s2)}"
          ...
          + "#play-studio .pl-vis .pl-badge{position:absolute;top:11px;left:11px;z-index:3;display:none;align-items:center;gap:7px;padding:4px 11px;border-radius:999px;font-size:var(--ha-fs,14px);font-weight:600;letter-spacing:.04em;color:#fff;background:rgba(0,0,0,.42);backdrop-filter:blur(8px)}"
          ...
          + "#play-studio .pl-name{flex:0 0 auto;font-size:var(--ha-fs,14px);font-weight:600;line-height:1.3;letter-spacing:-.006em}"
          + "#play-studio .pl-cat{flex:0 0 auto;align-self:flex-start;padding:5px 13px;border-radius:999px;font-size:var(--ha-fs,14px);background:rgba(255,255,255,.06);border:1px solid var(--holo-border,rgba(255,255,255,.11));color:var(--holo-ink-dim,#e2e2ee)}"
          + "#play-studio .pl-desc{flex:0 0 auto;font-size:var(--ha-fs,14px);line-height:1.5;color:var(--holo-ink-dim,#c2c2cc);display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}"
          + "#play-studio .pl-open{flex:0 0 auto;padding:12px;border:0;border-radius:10px;font:600 var(--ha-fs,14px) var(--win-font,system-ui);color:#fff;cursor:pointer;background:var(--accent,#5b8cff)}"
          + "#play-studio .pl-open:hover{filter:brightness(1.08)}"
          + "#play-studio .pl-open:active{transform:translateY(1px)}"
          + "#play-studio .pl-meta{flex:0 0 auto;display:flex;justify-content:space-between;font-size:var(--ha-fs,14px);color:var(--holo-ink-dim,#85858f)}"
          + "#play-studio .pl-k{cursor:pointer;font-family:ui-monospace,monospace}"
          + "#play-studio .pl-k:hover{color:var(--holo-ink,#e7e7ea)}"
          + "#play-studio .pl-rev{flex:0 0 auto}"
          + "#play-studio .pl-rev-h{font-size:var(--ha-fs,14px);font-weight:600;margin-top:4px}"
          + "#play-studio .pl-rev-e{font-size:var(--ha-fs,14px);color:var(--holo-ink-dim,#85858f)}";
        document.head.appendChild(st);
      }
...
<script>document.addEventListener("click",function(e){var a=e.target&&e.target.closest&&e.target.closest("a[data-url]");if(a){e.preventDefault();parent.postMessage({type:"holo-omni:go",value:a.getAttribute("data-url")},"*");}});<\/script></body></html>`;
      ...
      // Native tabs (host sets window.__holoNativeTabs via HOLO_CHROME_TABS). No JS in a holo:// page can CREATE a
      // Chrome tab (window.open strays; chrome.tabs.create is unreachable — content scripts don't inject on holo://).
      // So tabs are born from Chrome's own UI (Ctrl+T / + → NTP launcher) and an app click NAVIGATES the current
      // tab (location.assign) to the app. Phase A proved same-tab nav paints for `holo://<κ>` (bare kappa) and
      // `holo://os/<landing>`, but NOT `holo://space/<κ>` (that route is for in-page embedding) — so we map to the
      // proven forms only; everything else (space templates = multi-app tiles, async shapes) keeps the in-page
      // opener. See HOLO-NATIVE-TABS-GO-PROMPT.md + memory holo-native-tabs-blocked-cef149.
      if (window.__holoNativeTabs) {
        // Retire the double-bar: the native-hosted window has a real Chromium tabstrip + omnibox, so hide the
        // shell's own HTML tabstrip/nav/omnibox (native-chrome). The early head script (shell.html) adds this too;
        // re-add here in case the host flag landed after that script ran, so the launcher/home tab isn't double-barred.
        try { document.documentElement.classList.add("native-chrome"); document.documentElement.classList.add("native-tabs"); } catch (e) {}
        const _inPageOpen = window.HoloOpen;
        const _classify = (window.HoloOpenLib && window.HoloOpenLib.classifyOpen) || null;
        const nativeUrl = (ref) => {
          const v = String(ref == null ? "" : ref).trim(); if (!v) return null;
          if (/^holo:\/\/os\//i.test(v)) return v;                                   // already a servable OS/landing path
          const kind = _classify ? _classify(v).kind : "";
          if (kind === "kappa") return "holo://" + v.replace(/^did:holo:sha256:/i, "").replace(/^holo:\/\//i, "");   // proven ✓
          if (kind === "app") {                                                       // appId → catalog landing (proven ✓)
            try {
              const appId = v.replace(/^holo:\/\//i, "");
              const app = (typeof catalog !== "undefined" && catalog) ? catalog.find((a) => a.id === appId || a.id === v) : null;
              if (app && app.landing) return "holo://os" + (String(app.landing)[0] === "/" ? app.landing : "/" + app.landing);
            } catch (e) {}
            return null;                                                              // not resolvable → in-page fallback
          }
          return null;                                                                // space templates + words/cid/media/text → in-page
        };
        window.HoloOpen = function (ref) {
          try { const u = nativeUrl(ref); if (u) { location.assign(u); return Promise.resolve(true); } } catch (e) {}
          return _inPageOpen(ref);
        };
        window.HoloOpenInPage = _inPageOpen;   // in-page opener stays reachable (templates, non-navigational surfaces)
      }
      ...
      //    holospace window. Identity is content (Law L1), so the link resolves wherever it boots.
      {
        ...
        // consume-once (same idiom as resolveBootResume): strip the deep-link from the address bar, so a
        // reload / browser session-restore / autocompleted revisit paints the clean Home desktop — the
        // canonical URL never re-opens an app it already opened. Explicit links still open their app once.
        if (web || want) {
          for (const k of ["web", "open", "app", "shared", "shell"]) sp.delete(k);
          try { history.replaceState(null, "", location.pathname + (sp.toString() ? "?" + sp : "") + location.hash); } catch (e) {}
        }
      }
...
<body>
  ...
  <div class="doc">${md}</div>
      ...
      (function mountHintBar() {
        ...
        // The shortcuts hint is a PERMANENT desktop fixture — always visible on every tab, never
        // dismissable, so the entry point to the keymap is never lost. (Touch / narrow viewports hide it
        // via CSS, where a keyboard hint is meaningless — this is desktop-only.) `?` still opens the full
        // cheat sheet regardless.
        const sync = () => dot.classList.add("show");
        ...
        dot.addEventListener("click", (e) => {
          ...
          if (chip) { try { km.run(chip.dataset.run); } catch (x) {} return; }         // click a chip → run that command
          try { openCheat(); } catch (x) {}                                            // tap the dot (or no keyboard) → full cheat sheet
        });

        window.__syncHints = sync;
        window.__hintsHidden = () => false;   // always present now — kept so the cheat sheet footer reads a stable state
        sync();
      })();
      ...
      function openCheat() {
        ...
        cheat.querySelector(".sheet").innerHTML = `<div class="cheat-h">Keyboard shortcuts <span class="muted">· ${P.label}</span></div><div class="cheat-grid">`
          ...
          + `</div><div class="cheat-f">your keymap is content-addressed <span class="k">${keymapDid.replace("did:holo:sha256:", "holo://").slice(0, 30)}…</span> · share the link to sync your shortcuts</div>`;
