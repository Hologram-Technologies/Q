...
(function () {
  ...
  function mediaBounds() {
    ...
    // pill always rests INSIDE the visible canvas (never over the rail or off the bottom edge).
    ...
    // as CSS vars (rail width + envelope gap) — rebuild the canvas frame from them, so the pill still
    ...
    if (!b && DOC.querySelector(".holo-wa-root")) {
      ...
      b = { minX: railW + gap + MEDGE, minY: gap + MEDGE, maxX: W.innerWidth - gap - MEDGE, maxY: W.innerHeight - gap - MEDGE };
    }
    if (!b) {
  ...
  function placePill() {
    ...
    // align the pill's vertical CENTRE to the living Q orb's centre (desktop `.hw-q` OR the messenger home orb)
    // so the pill on the left and the orb on the right rest at one impeccable height.
    try { var orb = DOC.querySelector(".hw-q, .holo-home-orb"); if (orb) { var r = orb.getBoundingClientRect(); if (r.height) top = mClamp(Math.round(r.top + r.height / 2 - h / 2), b.minY, Math.max(b.minY, b.maxY - h)); } } catch (e) {}
    ...
    clampPillIntoFrame();                                          // cap width to the frame + settle inside it
  }
  ...
  // land the pill inside the CURRENT canvas frame — its position may predate a frame change (the frame
  // moved/shrank while the pill sat suspended/hidden, or the viewport resized).
  function clampPillIntoFrame() {
    if (!MWIN || !MWIN.el || !DOC.body.contains(MWIN.el)) return;
    var b = mediaBounds(), el = MWIN.el;
    el.style.boxSizing = "border-box";                             // cap the OUTER box (padding+border included)
    el.style.maxWidth = Math.max(160, b.maxX - b.minX) + "px";     // never wider than the frame itself
    el.style.left = mClamp(el.offsetLeft, b.minX, Math.max(b.minX, b.maxX - el.offsetWidth)) + "px";
    el.style.top = mClamp(el.offsetTop, b.minY, Math.max(b.minY, b.maxY - el.offsetHeight)) + "px";
  }
  // the canvas frame can move/shrink (viewport resize, expanded mode) — keep the pill inside it
  W.addEventListener("resize", function () {
    if (!MWIN || MWIN.suspended) return;
    clampPillIntoFrame();
  });
  ...
  function openWindow(w) {
    ...
    clampPillIntoFrame();                                          // ALWAYS inside the current canvas frame
