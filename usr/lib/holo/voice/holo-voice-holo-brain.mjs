...
const MODELS = {
  ...
  "qwen3.5-9b-thinking": specFor(PINNED.think.instant), // DEEP-REASONING tier (9B, capable WebGPU only — gated by holo-q-think-tier); request via createHoloModelBrain({model:"qwen3.5-9b-thinking"})
};
...
export function createHoloModelBrain(opts = {}) {
  ...
  async function ensure() {
    ...
    loadingP = (async () => {
      ...
      // THINK faculty (qwen35 9B HYBRID) runs on its OWN resident engine — the dense holo-brain-engine can't do
      // the gated-DeltaNet/attention hybrid arch. Same provider shape (load · generate · chat · info); weights
      // stream from the same κ-addressable .holo spec (specFor(PINNED.think.instant)). On-GPU resident decode.
      if (cfg.model === "qwen3.5-9b-thinking") {
        const spec = MODELS[cfg.model];
        const q = await import(/* @vite-ignore */ FORGE + "gpu/holo-qwen35-gpu-real.mjs");
        let engine = null;
        brain = {
          load: async (onProgress) => {
            if (!engine) { const L = await q.loadQwen35Brain({ holoUrl: spec.url, onProgress: onProgress || (() => {}) }); engine = L.resident || L; }
            info = { ready: true, model: cfg.model, device: "webgpu", resident: true }; return info;
          },
          generate: async function* (history, o = {}) { if (!engine) await brain.load(o.onProgress); yield* engine.generate(history, o); },
          chat: async (history, o = {}) => { if (!engine) await brain.load(o.onProgress); return engine.chat(history, o); },
          setSkill: () => ({ adapter: false, unsupported: true }), setAdapter: () => ({ adapter: false, unsupported: true }),
          info: () => info,
        };
        return brain;
      }
