...
import { reportTransport } from "./holo-transport-tier.mjs";   // P3: name the ACTIVE transport honestly
...
export function makeBroadcastTransport(name) {
  ...
  reportTransport({ tier: "loopback", via: "broadcastchannel", platform: name });   // P3: same-device local, no server
...
export function makeRTCTransport({ initiator, signaling, rtcConfig = RTC_CONFIG } = {}) {
  ...
  ready.then(() => reportTransport({ tier: "p2p", via: "webrtc-datachannel" })).catch(() => {});   // P3: direct device↔device
...
export async function makeDialHost({ rtcConfig = RTC_CONFIG } = {}) {
  ...
  channel.ready.then(() => reportTransport({ tier: "p2p", via: "holo-dial" })).catch(() => {});   // P3: cross-device, server-free
...
export async function joinDial(offerBlob, { rtcConfig = RTC_CONFIG } = {}) {
  ...
  pc.ondatachannel = (e) => { const w = wrapDataChannel(e.channel); w.ready.then(() => reportTransport({ tier: "p2p", via: "holo-dial" })).catch(() => {}); res(w); };   // P3
