...
// Returns { mode, ... } where mode ∈ { native, verified-torrent, drm-embed, deep-link, EXCLUDED }.
//
// `admit` (injected, default OFF) is the ONE posture knob: a torrent source is EXCLUDED unless admit(stream)
// approves it as a verified, lawful origin — in which case it becomes a first-class `verified-torrent` handle
// that the media resolver streams verify-on-read (P0/VS gate). With no `admit`, behavior is byte-identical to
// the historical hard line (torrents EXCLUDED) — so every existing caller is unchanged.
export function routeStream(stream, { admit = null } = {}) {
  if (!stream || typeof stream !== "object") return { mode: "EXCLUDED", reason: "empty" };
  const isTorrent = stream.infoHash || stream.fileIdx != null || /(^|\b)(magnet:|real-debrid|alldebrid|premiumize|debrid)/i.test(JSON.stringify(stream));
  if (isTorrent) {
    if (admit && admit(stream)) return { mode: "verified-torrent", kind: "torrent", infoHash: stream.infoHash || null, fileIdx: stream.fileIdx ?? null, sources: stream.sources || [] };
    return { mode: "EXCLUDED", reason: "torrent/debrid source — not admitted (no verified lawful origin)" };
  }
