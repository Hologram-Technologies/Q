...
import { makeGetBlock, resolveIpfsPath, directoryListingHtml, ipfsErrorHtml } from "./_shared/holo-ipfs-gateway.mjs";
...
async function serveIpfsView(rootCid, path, reqPath) {
...
self.addEventListener("fetch", (event) => {
  ...
  if (url.pathname.startsWith(VIEW)) {
    ...
    const m = rest.match(/^([^/]+)(\/.*)?$/);
    if (m) { event.respondWith(serveIpfsView(m[1], (m[2] || "").replace(/^\//, ""), url.pathname).catch(refused)); }
    return;
  }
