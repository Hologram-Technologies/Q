...
// This is R1+R2 of HOLO-SERVERLESS-RENDEZVOUS: the missing "first hello" that lights up the browser's
// device-mesh exit peer (holo-peer-egress.mjs) from ANY network — a phone on cellular pairs to a desktop
  ...
  try {
    while (Date.now() < deadline) {
      ...
      for (const blob of list) {
        const obj = await unseal(pairKappa, blob);          // refuses non-holder blobs (SEC-4) silently
        if (obj && obj.role === wantRole && obj.sdp && (!accept || accept(obj))) return obj;
      }
...
export async function offerSide({ pairKappa, mailbox, onChannel, timeoutMs = 30000, iceServers } = {}) {
  ...
  await mailbox.put(coord, await seal(pairKappa, { role: "offer", sdp: offerer.offer, ts: Date.now() }), Math.ceil(timeoutMs / 1000) + 30);
  const ans = await pollGet(mailbox, coord, pairKappa, "answer", { timeoutMs });
...
export async function answerSide({ pairKappa, mailbox, onChannel, timeoutMs = 60000, iceServers, accept = null } = {}) {
  const coord = coordinate(pairKappa);
  const off = await pollGet(mailbox, coord, pairKappa, "offer", { timeoutMs, accept });
  ...
  await mailbox.put(coord, await seal(pairKappa, { role: "answer", sdp: answerer.answer, ts: Date.now() }), Math.ceil(timeoutMs / 1000) + 30);
