...
const KIND = {
  // the OS's own health (boot wedge / renderer self-heal / serve storm), folded by holo-syshealth.mjs. The
  // ref IS Q's first-person message (already phrased there), so say() hands it through verbatim. Highest
  // salience (z:7) — the desktop itself having trouble outranks an app issue.
  "system.issue": { z: 7, risk: 0.85, say: (ref) => String(ref) },
