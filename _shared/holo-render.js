...
export async function configure({ importmap, base, resolver, route, bare, stream, media } = {}) {
  ...
  try { if (typeof window !== "undefined" && !window.HoloStream) (await import("./holo-stream-kit.mjs")).mount(window); } catch (e) {}   // auto-mount the unified streaming toolkit (window.HoloStream) — fail-open   // every configured surface streams the scene ahead (idle-time, deduped)
  return IMPORTMAP;
}
