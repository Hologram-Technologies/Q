...
(function () {
  ...
  function fromLegacy() {
    ...
    if (v === "immersive" || v === "full" || v === "theme") return { palette: "dark", immersive: true, wallpaper: "/usr/share/wallpapers/alps.jpg" };
  ...
  // Alpine Dawn backdrop (the alpine sunrise above a sea of cloud — the shared product default the
  // messenger also wears, so login · home · messenger match out of the box). Seeded ONCE into the
  // canonical state so the async engine (holo-theme.js) reads it as the saved choice and never overrides
  // this frame. It is a real κ/path image, so we set --holo-wallpaper and every boot screen (splash ·
  // login · shell) opens already wearing the photo.
  var FIRST_RUN = { palette: "dark", immersive: true, wallpaper: "/usr/share/wallpapers/alps.jpg" };
