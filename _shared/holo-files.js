...
import * as Mounts from "./holo-device-mounts.mjs";
...
async function listHome(path) {
  ...
  // collect handles first, then stat files in PARALLEL — a big folder must not pay N serial getFile()
  // round-trips (Native Mirror N4: 1k entries went from ~400ms serial → one batched pass).
  const dirs = [], files = [];
...
async function listDevice() {
  let doc; try { doc = await getJSON("/device/roots"); } catch { doc = null; }
  if (doc) return sortNodes((doc.entries || []).map((e) => node({
    name: e.name, path: "device:" + e.path, kind: "dir", source: "device-dir", role: "volume", _absPath: e.path,
  })));
  // NATIVE MIRROR (N1) — no host bridge (the static/web mount): This Device = the user's GRANTED
...
async function listMountDir(p) {
  const r = await Mounts.mountResolve(p); if (!r) return [];
  const dirs = [], files = [];
  for await (const [name, h] of r.dir.entries()) (h.kind === "directory" ? dirs : files).push([name, h]);
  const out = dirs.map(([name]) => node({ name, path: p + "/" + name, kind: "dir", source: "mount-dir", writable: true, role: "" }));
  const stat = await Promise.all(files.map(async ([name, h]) => {   // N4: parallel stat — real folders can be large
    let bytes = null, mtime = 0; try { const f = await h.getFile(); bytes = f.size; mtime = Math.floor(f.lastModified / 1000); } catch {}
    return node({ name, path: p + "/" + name, kind: "file", source: "mount", bytes, mtime, mime: mimeOf(name), writable: true, did: "" });
  }));
  return sortNodes(out.concat(stat));
}
// resolve a mount FILE node → its File (parent dir walk + getFileHandle)
async function mountFile(p) {
  const parts = String(p).split("/"); const name = parts.pop();
  const r = await Mounts.mountResolve(parts.join("/")); if (!r) throw new Error("mount not available");
  return (await r.dir.getFileHandle(name)).getFile();
}
// explicit-user-act writes on mounts (the ONLY writes — real files are sacred)
export async function mountMkdir(parentPath, name) { const r = await Mounts.mountResolve(parentPath); await r.dir.getDirectoryHandle(name, { create: true }); }
export async function mountCreateFile(parentPath, name, contents = "") {
  const r = await Mounts.mountResolve(parentPath); const fh = await r.dir.getFileHandle(name, { create: true });
  const w = await fh.createWritable(); await w.write(typeof contents === "string" ? contents : new Blob([contents])); await w.close();
}
export async function mountRemove(parentPath, name) { const r = await Mounts.mountResolve(parentPath); await r.dir.removeEntry(name, { recursive: true }); }
export async function mountRename(parentPath, oldName, newName) {
  const r = await Mounts.mountResolve(parentPath);
  let h, isFile = true;
  try { h = await r.dir.getFileHandle(oldName); } catch { h = await r.dir.getDirectoryHandle(oldName); isFile = false; }
  if (h.move) { try { await h.move(newName); return; } catch {} }
  if (!isFile) throw new Error("rename folder not supported here");
  const f = await h.getFile(); await mountCreateFile(parentPath, newName, await f.arrayBuffer()); await r.dir.removeEntry(oldName);
}
// the registry surface for the chrome (connect affordance, drop-to-mount, re-arm)
export const deviceMounts = Mounts;
